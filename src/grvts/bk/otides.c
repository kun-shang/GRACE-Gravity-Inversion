...
#include "grvts.h"
    ...
    int NMAJ_PER, NMAJ_OTS = 18, NALL_OTS  = 256;
    double *OTADM;
    OTSperturb *MAJ_PER;
    OTSconstit *ALL_OTS;
...
int ots_open (char *ots_name, int nmax)
{
    ...
    if ((fp_ot = fopen (ots_name,"r")) == NULL)
    {
...
//    printf ("NOTS = %d\n", i);
    ...
    printf ("NOTS = %d\n", NMAJ_PER);
//    NFES = 59462;
    MAJ_PER = (OTSperturb *) calloc ( NMAJ_PER, sizeof(OTSperturb));
    ...
    while (1)
    {
        ...
        sscanf (string, "%lf%s%d%d%lf%lf%lf%lf", 
            &MAJ_PER[i].ds, MAJ_PER[i].name, &MAJ_PER[i].n, &MAJ_PER[i].m, 
            &MAJ_PER[i].cp, &MAJ_PER[i].sp, &MAJ_PER[i].cm, &MAJ_PER[i].sm);    

        MAJ_PER[i].argn[0] = (int)(MAJ_PER[i].ds/100)%10;
        MAJ_PER[i].argn[1] = (int)(MAJ_PER[i].ds/10)%10 - 5;
        MAJ_PER[i].argn[2] = (int)(MAJ_PER[i].ds/1)%10 - 5;
        MAJ_PER[i].argn[3] = (int)(MAJ_PER[i].ds*10)%10 - 5;
        MAJ_PER[i].argn[4] = (int)(MAJ_PER[i].ds*100)%10 - 5;
        MAJ_PER[i].argn[5] = (int)(MAJ_PER[i].ds*1000)%10 - 5;
        MAJ_PER[i].fnm = 1e-11;
        ...
        double *coef)
{
    ...
    for (i = 0; i < NMAJ_PER; i++)
    {
        if (MAJ_PER[i].n > nmax) 
//        if (MAJ_PER[i].n > nmax || MAJ_PER[i].n < 2) 
        {
            continue;
        }
...
//        arg2theta (info->jdt, info->gmst, MAJ_PER[i].argn, &ang);
        ...
        {       
            arg2theta (jdt, gmst, MAJ_PER[i].argn, &ang);
            cosang = cos(ang);
            sinang = sin(ang);
        }
        else 
