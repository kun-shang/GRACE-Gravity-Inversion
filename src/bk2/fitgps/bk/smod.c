...
//        sscanf (string, "%d%d%lf%lf%*d%*d%lf%lf%*d%*d%lf%lf%*d%*d%lf%lf%*d%*d%lf%lf", 
...
//            coef[ind + n - m] = c; coef[ind + n - m + l] = s;
...
{
    int i, n, m, l, ind, dim_x;
    double step_ot, tjd[2], tt;
    InfStruct info;

    dim_x = (NOMAX + 1) * (NOMAX + 1) + 1;

    step_ot = 86400 / DIM_OT;
        ...
        double *dadr, double *dadk2)
{
    ...
    double jd0, tt, utc, te[9], tx[3], ty[3], tz[3], ao[3], as[3], ag[3], dadk2e[3], t, aa[3],
...
/*
    ...
    {
        t = info.gps / 86400.0;
        lgr_order (AOD_EPH, 5, (NDMAX + 1) * (NDMAX + 1) + 1 , t, COEFD, 1);
...
*/
    cs2ada (llr, COEF, NMAX, ae, part, dadre, 0);
...
double sumcs (double *cs, double *cs1, int nmax, int nmax1)
...
double csnm (double *cs, int nmax, int n, int m, double *c, double *s)
{
    int ind, l;
...
{
    double doodarg[6], ang, cp, sp, cm, sm, fnm;
    int i, ncon = 1, n,m, l, ind;
...
//        printf ("cp = %e\t sp = %e\t ang = %e\t doodarg = %e\n", cp, sp, ang, doodarg[3]);
        ...
        arg2theta (jdt, gmst, otfes[i].argn, &ang);
...
//            m1, m2, xp, yp, xpm, ypm);
    ...
    label = 1;
    *c21p = -1.333e-9 * (m1 + label * 0.0115 * m2);
    *s21p = -1.333e-9 * (m2 - label * 0.0115 * m1);
...
{
    int n, lps, i;
    double gps0, h, *eph, *dxdp, *dodpo, *dodpd, *dodpp, xc2[6], dxdx0[36], dodx[6], 
        dodx0[36], tjd[2], xsc[6], dv[3];
