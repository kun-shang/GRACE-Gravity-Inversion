...
{
    FILE *fp_stdin, *fp_simu, *fp_result, *fp_res, *fp_otbin, *fp_atbin,*fp_par;
    short int year, month, day, error, de_num;
    int iter, part, all, teout, accid;
    int n, i, j, gps, ndata, slvls, genrel, maxiter, step_ot, step_at,
        lps, mjds, kpl, ntide, indicator;
    double pi[3], pe[3], llh[3], tt, tts, tte, tp, h, hcut, sigma2, sigma;

    double utc, utcs, utce, ts_sim, te_sim, t_sim, xsim[6], xobs[6], 
        start, finish, duration, xsm[6], xau[6], xtm[6], x2[6], dist, velt, 
        calculable, nsp, nsv, azimuth, elevation, elcutoff, data_value, 
        rmsold, rmsnew, rms2n, step_day, jd_sel, ts_sel, te_sel, rmsv[6] = {0},
        *state, *bi, *bit, *btbi, *btyi, *BTry, *BTrB, *xbi,
        *p, *px, *dx, *x0, *xnew, *xold, weight[6], dyi[1], tcdop, 
