...
static int8_t SccsId[] = "$Id: PutCharBits.c,v 1.3 2009/06/06 22:15:38 glk Exp $";
...
/
/Input variables:
/   x  input int8_t from which the bits are to be taken
/   px starting position of the bits, with 0 being the lsb.
/   n  number of bits to put.
/Output variables:
/   y  output int8_t where the bits are put
...
/
/********************************************************************/
int32_t            PutCharBits(uint8_t x, int32_t px,
                   uint8_t *y, int32_t py, int32_t n)
{
   uint8_t   a, b, c;
